...
public class GridAnimationManager : MonoBehaviour
{
    ...
    private Vector3 startObjPos; //record start objects position before the animation to later make sure rigidbodies didnt mess up position
    private Vector3 endObjPos; //record end objects position before the animation to later make sure rigidbodies didnt mess up position
    ...
    IEnumerator WaitForCubeAnimation(LinkedList<GameObject> objectsToAnimate, bool foundEnd, LinkedList<GameObject> path)
    {
        while(objectsToAnimate.Count > 0)
        {
            ...
            if (gridTopMatrix[row,col]!= null && gridTopMatrix[row,col].name[1] == 'e')
            {
                ...
                //change alpha of the weight to see the cube color underneither better
